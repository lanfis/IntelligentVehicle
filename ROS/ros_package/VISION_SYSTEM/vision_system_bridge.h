#pragma once
#ifndef _VISION_SYSTEM_BRIDGE_H_
#define _VISION_SYSTEM_BRIDGE_H_
...
Vision_System_Bridge::Vision_System_Bridge() : it_(n_)
{
  ver_ = 1.0;
...
void Vision_System_Bridge::pub_topic_get()
{
    topic_device_id_pub = device_id_pub_.getTopic();
    topic_width_pub = width_pub_.getTopic();
    topic_height_pub = height_pub_.getTopic();
    topic_activation_pub = activation_pub_.getTopic();
}
...
void Vision_System_Bridge::pub_shutdown()
{
    device_id_pub_.shutdown();
    width_pub_.shutdown();
    height_pub_.shutdown();
}

void Vision_System_Bridge::pub_topic_get()
{
    topic_image_color_sub = image_color_sub_.getTopic();
    topic_image_depth_sub = image_depth_sub_.getTopic();
    topic_camera_info_sub = camera_info_sub_.getTopic();
}
...
void Vision_System_Bridge::sub_shutdown()
{
    image_color_sub_.shutdown();
    image_depth_sub_.shutdown();
